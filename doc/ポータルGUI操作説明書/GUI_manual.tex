...
プロジェクト編集画面では,プロジェクトディレクトリ内のファイルの閲覧,編集,ワークフロー実行制御およびコンソールによるログ確認を行います.
...
ファイル一覧からディレクトリを選択すると,そのディレクトリ内のファイルがファイル一覧に表示されます.上の階層に戻る場合は「..」を押します.また,テキスト形式のファイルを選択すると,それらのファイルの内容が右のペインに表示され,編集を行うことが出来ます.
...
\section{ワークフローの実行}
「pwl」「pwf」などのワークフローファイルを開いている状態で,「RUN」ボタンを押すと,ワークフローが実行されます.
\section{ワークフローの停止}
ワークフローの実行中に「STOP」ボタンを押すと,実行中のワークフローを停止出来ます.
\section{コンソールの表示}
「Output」ボタンを押すと,ポータルGUIの動作しているホスト上のコンソールが表示されます.
再度「Output」ボタンを押すと閉じます.
...
「Vim Mode」/「Xcode Mode」ボタンで,エディタ部分のキーボードバインドが変更出来ます.
